* Pintos: Métodos de sincronización para la llamada al sistema ~exec~
...
5. Al hilo ~initial_thread~ se le debe también de asociar la meta información
   ...
   ~thread_init ()~.
6. Revisar a conciencia el documento de diseño para comprender qué casos especiales
   es necesario considerar como parte de la solución a ~wait~.

#+CAPTION: Árbol de procesos
#+NAME:   fig:1
[[./assets/notas-lab-oct24-pintos-process-control-syscalls/process-tree.png]]
